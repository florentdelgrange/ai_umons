"""
Based on a finetuned inceptionV3 model (https://github.com/sohaib-l/finetuning_inceptionv3)
To be used with the "openu" dataset (https://goo.gl/a4aHXR)
"""
...
def train(train_dir, val_dir, output_model_file, nb_epoch, batch_size, weights_dir, verbose=True):
    ...
    '''
    checkpointer = ModelCheckpoint(filepath=os.path.join(module_path, weights_dir + '/weights_tl_tmp.h5'), verbose=0,
                                   save_best_only=True)
    ...
    checkpointer = ModelCheckpoint(filepath=os.path.join(module_path, weights_dir + '/weights_ft_tmp.h5'), verbose=0,
                                   save_best_only=True)
...
def training(path_to_dataset, output_model_dir="weights_gender_and_age/", output_model_file="weights.h5",
    ...
    if path_to_dataset is None:
        ...
        print("Help: your dataset folder should contain two folders: 'train' and 'valid'.")
        sys.exit(1)
    ...
    train(train_dir, val_dir, output_model_file, nb_epoch, batch_size, weights_dir=output_model_dir)


if __name__ == "__main__":
    if not os.path.exists("weights_gender_and_age"):
        os.makedirs("weights_gender_and_age")
    training(path_to_dataset="sorted_gender_and_age", output_model_dir="weights_gender_and_age/",
             output_model_file="weights.h5")
